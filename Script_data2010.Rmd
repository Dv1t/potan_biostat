...
Data <- read_xlsx ("Data/Raw/2010.xlsx")
#glimpse(Data)
summary(Data)
Data %>% as.tibble()
```
...
```
```{r}
table(Data_en$gender, useNA = "always") # есть 3NA
table(Data_en$case, useNA = "always") # есть 1NA
summary (Data_en$case, useNA = "always")
sum (is.na(Data_en$case))

# отбираем наблюдения с указанным полом "м" или "ж"  (-3 наблюдений) и с наличием протокола 

Data_step1 <- Data_en %>%
   filter(!is.na(case)) %>%
  filter(gender %in% c("м","ж")) %>% 
  filter(as.numeric(age_days) <= (1)) # удаляем наблюдения со значением переменной > 1
#на данном этапе остается 750 наблюдений
...
table(Data_step1$gest, useNA = "always") #удаляем 3 NA
table(Data_step1$mac, useNA = "always") #удаляем 258 "да" 
table(Data_step1$CM, useNA = "always") # из оставшихся токлько 424 без CM
table(Data_step1$IUGR, useNA = "always") # из них 424 без IUGR
table(Data_step1$MP, useNA = "always") 
...
Data_step2 <- Data_step1 %>% 
  ...
  filter(is.na(CM)) %>% 
  filter(is.na(IUGR)) %>% 
  filter(is.na(MP)) # 416 одноплодных

#нет значений веса более 5000 г, фильтрация по этому показателю не требуется
Data_step2$WB <- as.numeric(Data_step2$WB)
summary(Data_step2$WB)

# фильтрация по неделям гестации не требуется, все значения соответствуют парметру >= 14 недель
table (Data_step2$gest,  useNA = "always")

#Удаляем непонятную переменную ...24
#Data_step2 <- Data_step2 %>% select(-c(...24))
#summary(Data_step2)

#приводим still_live к бинарному виду (мертворожденный-0, живорожденный -1

Data_step2 <- Data_step2 %>% 
   mutate(still_live = ifelse(age_days == 0, 0, 1))
...
Data_step2$gender <- as.factor(Data_step2$gender) #gender

Data_step2 <- Data_step2 %>% 
  mutate(still_live = ifelse(still_live == 1, "live", "still"))
Data_step2$still_live  <- as.factor(Data_step2$still_live) #still_live 
...
Data_step2 <- Data_step2 %>% add_column(year = "2010", .before = 1)
